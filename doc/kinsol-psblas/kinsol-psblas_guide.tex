...
\end{fulltable}
	...
	\begin{description}
		...
		\item[\textcolor{mPurple}{\texttt{"AS"}}] Additive Schwarz (AS), with overlap 1 and
		ILU(0) on the local blocks.
		\item[\textcolor{mPurple}{\texttt{"ML"}}] this enables the construction of a multilevel preconditioner for which all the details for these preconditioner are described in Section~\ref{sec:mld2p4}, and are based on the ML2DP4 package~\cite{MR2738234,mld2p4guide}.
	\end{description}
...
All this construction is what is usually called the build-phase of the hierarchy of AMG components, i.e., the so-called build phase of the preconditioner.
